...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", help="Output file directory", type=str) # /mnt/data1/syjintw/MMSys25_extension/dataset/preprocessed
    ...
    for offset in range(1, total_frames):
        ...
        # res4
        tmp_res4_gs = io_3dgs.GaussianModelV2(Path(args.input)/f"{scene_name}"/"opacity"/f"{scene_name}_res4"/f"{frame}"/"point_cloud"/"iteration_30000"/"point_cloud.ply")
        res4_gs.add_attribute(f"x_{offset}", "f4", tmp_res4_gs.data["x"]["data"])
        res4_gs.add_attribute(f"y_{offset}", "f4", tmp_res4_gs.data["y"]["data"])
        res4_gs.add_attribute(f"z_{offset}", "f4", tmp_res4_gs.data["z"]["data"])

        # res2
        tmp_res2_gs = io_3dgs.GaussianModelV2(Path(args.input)/f"{scene_name}"/"opacity"/f"{scene_name}_res2"/f"{frame}"/"point_cloud"/"iteration_30000"/"point_cloud.ply")
        res2_gs.add_attribute(f"x_{offset}", "f4", tmp_res2_gs.data["x"]["data"])
        res2_gs.add_attribute(f"y_{offset}", "f4", tmp_res2_gs.data["y"]["data"])
        res2_gs.add_attribute(f"z_{offset}", "f4", tmp_res2_gs.data["z"]["data"])

        # res1
        tmp_res1_gs = io_3dgs.GaussianModelV2(Path(args.input)/f"{scene_name}"/"opacity"/f"{scene_name}_res1"/f"{frame}"/"point_cloud"/"iteration_30000"/"point_cloud.ply")
        res1_gs.add_attribute(f"x_{offset}", "f4", tmp_res1_gs.data["x"]["data"])
        res1_gs.add_attribute(f"y_{offset}", "f4", tmp_res1_gs.data["y"]["data"])
        res1_gs.add_attribute(f"z_{offset}", "f4", tmp_res1_gs.data["z"]["data"])

    # Handle multiple opacity
    ...
    lod0_save_dir = Path(args.output)/f"{scene_name}"/"0"/"point_cloud"/"iteration_30000"
    lod0_save_dir.mkdir(parents=True, exist_ok=True)
    lod0_gs.export_gs_to_ply(lod0_save_dir/"point_cloud.ply")

    lod1_save_dir = Path(args.output)/f"{scene_name}"/"1"/"point_cloud"/"iteration_30000"
    lod1_save_dir.mkdir(parents=True, exist_ok=True)
    lod1_gs.export_gs_to_ply(lod1_save_dir/"point_cloud.ply")

    lod2_save_dir = Path(args.output)/f"{scene_name}"/"2"/"point_cloud"/"iteration_30000"
